...
Sử dụng mô hình Logistic Regression & 8 biến Beneish M-Score
Tác giả: Chuyên gia Lập trình Web & Khoa học Dữ liệu
...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Dự Báo Gian Lận BCTC | Beneish M-Score & Logistic Regression",
    page_icon="⚖️",
    layout="wide",
...
def load_default_data():
    """Tự động tìm và đọc file dữ liệu mặc định MScore_data.csv"""
    possible_paths = [
        "MScore_data.csv",
...
def clean_data(df):
    """Làm sạch và kiểm tra dữ liệu theo chuẩn mô hình"""
...
def train_model(data, test_size=0.20, random_state=42):
    """Huấn luyện mô hình Pipeline: StandardScaler + LogisticRegression"""
    X = data[FEATURES]
    y = data[TARGET]
...
def compute_metrics(y_true, y_prob, threshold=0.5):
    """Tính toán đầy đủ các chỉ số đánh giá theo ngưỡng phân loại threshold"""
...
def calculate_beneish_mscore(dsri, gmi, aqi, sgi, depi, sgai, tata, lvgi):
    """Tính điểm Beneish M-Score nguyên bản (Beneish 1999 8-variable model)"""
    m_score = (
        -4.84
        + 0.920 * dsri
        + 0.528 * gmi
...
uploaded_file = st.sidebar.file_uploader("Tải lên file CSV dữ liệu mới (nếu có)", type=["csv"])
...
if uploaded_file is not None:
    ...
    if raw_df is not None:
        st.sidebar.info("Đang sử dụng dữ liệu mặc định: `MScore_data.csv`")
    else:
...
if raw_df is None:
    ...
    st.error("⚠️ Không tìm thấy file dữ liệu `MScore_data.csv`. Vui lòng tải lên file CSV ở thanh bên trái (Sidebar) để tiếp tục!")
    st.stop()
...
# 2. Tham số mô hình
st.sidebar.subheader("2. Tham số Mô hình")
test_size_ratio = st.sidebar.slider("Tỷ lệ tập kiểm tra (Test size)", min_value=0.10, max_value=0.40, value=0.20, step=0.05)
random_seed = st.sidebar.number_input("Random Seed (để tái lập kết quả)", value=42, step=1)
...
# Huấn luyện mô hình
pipeline, X_train, X_test, y_train, y_test, y_test_prob, intercept, coef, coef_df = train_model(
    data, test_size=test_size_ratio, random_state=random_seed
)
...
st.markdown('<div class="sub-header">Ứng dụng Học máy kết hợp 8 chỉ số Beneish M-Score & Hồi quy Logistic giúp phát hiện sớm rủi ro thao túng BCTC</div>', unsafe_allow_html=True)
...
# =============================================================================
with tab1:
    st.subheader("1. Tổng quan Bộ dữ liệu")
...
# =============================================================================
with tab2:
    st.subheader("2. Kết quả Huấn luyện & Đánh giá Mô hình Logistic Regression")
    ...
    # Tạo diễn giải chi tiết
    interpretation_list = []
    for name, b in zip(FEATURES, coef):
        or_val = np.exp(b)
        if b > 0:
            meaning = f"{name} tăng 1 độ lệch chuẩn làm tăng log-odds gian lận {b:.4f}; odds gian lận được nhân {or_val:.4f}, giữ các biến khác không đổi."
        else:
            meaning = f"{name} tăng 1 độ lệch chuẩn làm giảm log-odds gian lận {abs(b):.4f}; odds gian lận được nhân {or_val:.4f}, giữ các biến khác không đổi."
        interpretation_list.append({
            "Chỉ số": name,
            "Hệ số": b,
            "Odds Ratio": or_val,
            "Ý nghĩa kinh tế": meaning
        })
    ...
    with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
        ...
        pd.DataFrame({
            "Thông số": ["Intercept", "Threshold", "Train_size", "Test_size", "ROC_AUC"],
            "Giá trị": [intercept, threshold, len(X_train), len(X_test), roc_auc_val]
...
# =============================================================================
with tab3:
    st.subheader("3. Dự báo Rủi ro Gian lận cho Doanh nghiệp Cụ thể")
    ...
    if submit_single:
        ...
        # Dự báo xác suất bằng Pipeline Logistic Regression
        fraud_proba = float(pipeline.predict_proba(input_df)[0, 1])
        is_fraud = fraud_proba >= threshold

        # Tính toán thêm chỉ số Beneish M-Score truyền thống
        ...
        with col_res1:
            if is_fraud:
                ...
                <div class="alert-danger-box">
                    ...
                    <p style="margin:0; font-size: 1.05rem;">
                        Mô hình Logistic Regression ước tính xác suất gian lận BCTC của doanh nghiệp này là 
            ...
            # Đồng hồ đo Gauge Chart xác suất
            gauge_fig = go.Figure(go.Indicator(
                mode="gauge+number",
                value=fraud_proba * 100,
                domain={'x': [0, 1], 'y': [0, 1]},
                title={'text': "Xác suất Gian lận P(Fraud) (%)", 'font': {'size': 18}},
                number={'suffix': "%"},
                gauge={
                    'axis': {'range': [0, 100]},
        ...
        with col_res2:
            ...
            if is_fraud == is_beneish_manipulator:
                st.success("✅ **Sự đồng thuận cao:** Cả 2 mô hình (Logistic Regression và Beneish M-Score) đều đưa ra kết luận thống nhất!")
            else:
                st.warning("⚠️ **Lưu ý khác biệt:** Có sự khác biệt giữa mô hình Beneish chuẩn hóa theo bộ dữ liệu thực tế (Logistic Regression) và công thức gốc năm 1999. Khuyến nghị kiểm toán viên đào sâu thêm các khoản mục trọng yếu.")

            # Biểu đồ so sánh chỉ số của DN so với trung bình
...
# =============================================================================
with tab4:
    st.subheader("4. Dự báo Rủi ro Gian lận Hàng loạt từ File CSV / Excel")
...
# =============================================================================
with tab5:
    st.subheader("5. Cơ sở Lý thuyết & Hướng dẫn Sử dụng Hệ thống")
    
    st.markdown("""
    ### 🏛️ Mô hình Beneish M-Score là gì?
    Mô hình **Beneish M-Score** được Giáo sư Messod Beneish phát triển vào năm 1999. Đây là một công cụ phân tích định lượng kết hợp 8 tỷ số tài chính được tính toán từ Báo cáo tài chính (BCTC) nhằm phát hiện khả năng một doanh nghiệp đang **thao túng lợi nhuận (Earnings Manipulation)** hoặc **gian lận BCTC**.
    ...
    ### 🤖 Ưu thế của Mô hình Hồi quy Logistic trên Hệ thống:
    1. **Tối ưu hóa theo dữ liệu thực nghiệm:** Mô hình học máy Logistic Regression được chuẩn hóa (`StandardScaler`) và tối ưu trọng số dựa trên bộ dữ liệu kiểm toán thực tế thay vì áp đặt hệ số cố định từ năm 1999.
    2. **Xác suất mềm (Calibrated Probabilities):** Cung cấp xác suất rủi ro cụ thể $P(\\text{Fraud}) \\in [0, 1]$, giúp kiểm toán viên và nhà đầu tư có thước đo định lượng rõ ràng.
    3. **Tùy biến ngưỡng quyết định (Flexible Threshold):** Trong bài toán kiểm toán và phòng chống gian lận, việc **bỏ sót gian lận (False Negative)** thường gây hậu quả nghiêm trọng hơn rất nhiều so với cảnh báo nhầm (False Positive). Người dùng có thể chủ động hạ ngưỡng xác suất để tăng độ nhạy (Recall).
    """)
...
<div style="text-align: center; color: #94A3B8; font-size: 0.88rem;">
    ⚖️ <b>Hệ thống Dự báo Gian lận Báo cáo Tài chính</b> | Xây dựng trên nền tảng Streamlit & Scikit-Learn.<br>
